...
class FormattersInitializer:
    def __init__(self, stdout, cwd, args):
        ...
        self.formatters = []
        self.silent = getattr(args, 'silent') if hasattr(args, 'silent') else False
        self.silent_progress = getattr(args, 'silent_progress') if hasattr(args, 'silent_progress') else False
        ...
        for fc in Formatter.__subclasses__():
            if fc.DEFAULT:
                ...
                if not found:
                    ...
                    await self.initialize_formatter(event_broadcaster, split(name), strict, format_options)
        for f in self.formatters:

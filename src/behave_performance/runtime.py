...
class BehavePerformance(object):
    ...
    async def configure(self):
        format_options = {
            "colors_enabled": not self.args.no_format_color,
            "cwd": os.getcwd(),
        }
        ...
        for arg in BEHAVE_ARGS:
            value_arg = getattr(self.args, arg) if hasattr(
                self.args, arg) else None
            if arg == 'filename':
                self.bargs[arg] = os.path.realpath(value_arg)
            elif value_arg and value_arg != None:
                # bargs.append('--{}'.format(arg.replace('_', '-')))
                # bargs.append(value_arg)
                self.bargs[arg] = value_arg
        #Turn off all verbosity
        self.bargs['summary'] = False
    ...
    async def run(self):
        ...
        for i, sim in enumerate(self.simulations):
            ...
            self.ee.emit(PERF_EVENTS.SIMULATION_RUN_STARTED,sim["veggie"]["name"],sim_result.start)
            ...
            if self.ramp_up is not None:
                self.ee.emit(PERF_EVENTS.RAMP_STARTED,"up")
                self.cur_percent = 0
            ...
            for i in range(len(cnt_per_proc)):
                p = Process(target=manager, args=[
                            i, cnt_per_proc[i], self.task_queues[i], result_queue, self.bargs])
            ...
            while listening:
                if not result_queue.empty():
                    try:
                        ...
                        event['result']=result
                        self.ee.emit(
                            PERF_EVENTS.CUKE_RUN_FINISHED, event)
            ...
            self.ee.emit(PERF_EVENTS.SIMULATION_RUN_FINISHED,sim_result)
            ...
            if self.end_ramp is None:
                ...
                if (self.begin_end is not None and cur_time > self.begin_end) or (self.ran >= self.max_ran and self.begin_end is None):
                    if self.ramp_down is None:
                        ...
                        self.ee.emit(PERF_EVENTS.RAMP_STARTED, "down")
    ...
    async def __manage_process(self, p_id: int):
        ...
        for l in range(len(gs)):
            ...
            if pg["running"] < int(pg["runners"]) and (self.scheduled_runtime is not None or pg["ran"] + pg["running"] < int(pg["count"])):
                ...
                self.running += 1
                self.processes[p_id][0] -= 1
                self.ee.emit(PERF_EVENTS.CUKE_RUN_STARTED, dict(pg))
        ...
        if self.end_ramp is not None:
            if cur_time > self.end_ramp:
                ...
                if self.ramp_up is None:
                    self.ee.emit(PERF_EVENTS.RAMP_FINISHED,"down")
                    self.ramp_down = None
                    self.executing = False
                    self.cur_percent = 0
                else:
                    self.ee.emit(PERF_EVENTS.RAMP_FINISHED,"up")
                    self.ramp_up = None
                    self.cur_percent = 100
                self.ramper.cancel()

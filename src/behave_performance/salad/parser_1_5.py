...
RULE_TYPE = [
    ...
    '_SimulationSampleLine',  # #SimulationSampleLine
    '_SynchronizedLine',  # #SynchronizedLine
    '_TimeLine',  # #TimeLine
    '_GroupLine',  # #GroupLine
    '_PopulationLine',  # #PopulationLine
    ...
    'Simulation_Definition',  # Simulation_Definition! := Tags? (Simulation | SimulationPeriod | SimulationSample)
    'Simulation',  # Simulation! := #SimulationLine Description_Helper Group* RampUp? RampDown? RandomWait?
    'SimulationPeriod',  # SimulationPeriod! := #SimulationPeriodLine Description_Helper Group* Time RampUp? RampDown? RandomWait?
    'SimulationSample',  # SimulationSample! := #SimulationSampleLine Description_Helper Population* Time TotalRunners RampUp? RampDown? RandomWait?
    ...
    'Group_Arg',  # Group_Arg := DataTable? DocString? Runners Count? Start? Stop? Synchronized?
    'Population',  # Population! := #PopulationLine Population_Arg?
    'Population_Arg',  # Population_Arg := DataTable? DocString? Percentage Start? Stop? Synchronized?
...
class Parser(object):
    ...
    def match_SimulationSampleLine(self, context, token):
        if token.eof():
            return False
        return self.handle_external_error(context, False, token, context.token_matcher.match_SimulationSampleLine)
    ...
    def match_PopulationLine(self, context, token):
        if token.eof():
            return False
        return self.handle_external_error(context, False, token, context.token_matcher.match_PopulationLine)
    ...
    def match_token(self, state, token, context):
        state_map = {
            ...
            54: self.match_token_at_54,
            55: self.match_token_at_55,
            56: self.match_token_at_56,
            57: self.match_token_at_57,
            58: self.match_token_at_58,
            59: self.match_token_at_59,
            60: self.match_token_at_60,
            61: self.match_token_at_61,
            62: self.match_token_at_62,
            63: self.match_token_at_63,
            64: self.match_token_at_64,
            65: self.match_token_at_65,
        }
    ...
    def match_token_at_3(self, token, context):
        ...
        if self.match_SimulationPeriodLine(context, token):
                ...
                self.build(context, token)
                return 30
        if self.match_SimulationSampleLine(context, token):
                self.end_rule(context, 'Plan_Header')
                self.start_rule(context, 'Simulation_Definition')
                self.start_rule(context, 'SimulationSample')
                self.build(context, token)
                return 48
        ...
        token.detach
        expected_tokens = ["#EOF", "#Empty", "#Comment", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_4(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_5(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_6(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Empty", "#Comment", "#GroupTypeLine", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_7(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#GroupTypeLine", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_8(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#GroupTypeLine", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_9(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#TableRow", "#DocStringSeparator", "#FeaturesLine", "#GroupTypeLine", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_13(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#GroupTypeLine", "#GroupsLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_14(self, token, context):
        ...
        token.detach
        expected_tokens = ["#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_15(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Empty", "#Comment", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_16(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_17(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_18(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#TableRow", "#DocStringSeparator", "#RunnersLine", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_19(self, token, context):
        ...
        token.detach
        expected_tokens = ["#TableRow", "#DocStringSeparator", "#RunnersLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_21(self, token, context):
        ...
        token.detach
        expected_tokens = ["#RunnersLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 21
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:2>Group:1>Group_Arg:2>Runners:0>#RunnersLine:0
    def match_token_at_22(self, token, context):
        ...
        state_comment = "State: 22 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:2>Group:1>Group_Arg:2>Runners:0>#RunnersLine:0"
        token.detach
        expected_tokens = ["#EOF", "#CountLine", "#StartLine", "#StopLine", "#SynchronizedLine", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_23(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#StartLine", "#StopLine", "#SynchronizedLine", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_24(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#StopLine", "#SynchronizedLine", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_25(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#SynchronizedLine", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_26(self, token, context):
        ...
        token.detach
        expected_tokens = ["#EOF", "#GroupLine", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 26
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:3>RampUp:0>#RampUpLine:0
    def match_token_at_27(self, token, context):
        ...
        state_comment = "State: 27 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:3>RampUp:0>#RampUpLine:0"
        token.detach
        expected_tokens = ["#EOF", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 27
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:4>RampDown:0>#RampDownLine:0
    def match_token_at_28(self, token, context):
        ...
        state_comment = "State: 28 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:4>RampDown:0>#RampDownLine:0"
        token.detach
        expected_tokens = ["#EOF", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 28
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:5>RandomWait:0>#RandomWaitLine:0
    def match_token_at_29(self, token, context):
        ...
        state_comment = "State: 29 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:0>Simulation:5>RandomWait:0>#RandomWaitLine:0"
        token.detach
        expected_tokens = ["#EOF", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_30(self, token, context):
        ...
        state_comment = "State: 30 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:0>#SimulationPeriodLine:0"
        token.detach
    ...
    def match_token_at_31(self, token, context):
        ...
        state_comment = "State: 31 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:1>Description_Helper:1>Description:0>#Other:0"
        token.detach
    ...
    def match_token_at_32(self, token, context):
        ...
        state_comment = "State: 32 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:1>Description_Helper:2>#Comment:0"
        token.detach
    ...
    def match_token_at_33(self, token, context):
        ...
        state_comment = "State: 33 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:0>#GroupLine:0"
        token.detach
        expected_tokens = ["#TableRow", "#DocStringSeparator", "#RunnersLine", "#GroupLine", "#TimeLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_34(self, token, context):
        ...
        state_comment = "State: 34 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:0>DataTable:0>#TableRow:0"
        token.detach
        expected_tokens = ["#TableRow", "#DocStringSeparator", "#RunnersLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
    ...
    def match_token_at_35(self, token, context):
        ...
        state_comment = "State: 35 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:1>DocString:0>#DocStringSeparator:0"
        token.detach
        expected_tokens = ["#DocStringSeparator", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
    ...
    def match_token_at_36(self, token, context):
        ...
        state_comment = "State: 36 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:1>DocString:2>#DocStringSeparator:0"
        token.detach
        expected_tokens = ["#RunnersLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
        ...
        self.add_error(context, error)
        return 36
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:2>Runners:0>#RunnersLine:0
    def match_token_at_37(self, token, context):
        ...
        state_comment = "State: 37 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:2>Runners:0>#RunnersLine:0"
        token.detach
    ...
    def match_token_at_38(self, token, context):
        ...
        state_comment = "State: 38 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:3>Count:0>#CountLine:0"
        token.detach
    ...
    def match_token_at_39(self, token, context):
        ...
        state_comment = "State: 39 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:4>Start:0>#StartLine:0"
        token.detach
    ...
    def match_token_at_40(self, token, context):
        ...
        state_comment = "State: 40 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:5>Stop:0>#StopLine:0"
        token.detach
    ...
    def match_token_at_41(self, token, context):
        ...
        state_comment = "State: 41 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:2>Group:1>Group_Arg:6>Synchronized:0>#SynchronizedLine:0"
        token.detach
    ...
    def match_token_at_42(self, token, context):
        ...
        state_comment = "State: 42 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:3>Time:0>#TimeLine:0"
        token.detach
        expected_tokens = ["#EOF", "#Empty", "#Comment", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_43(self, token, context):
        ...
        state_comment = "State: 43 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:3>Time:1>Description_Helper:1>Description:0>#Other:0"
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
    ...
    def match_token_at_44(self, token, context):
        ...
        state_comment = "State: 44 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:3>Time:1>Description_Helper:2>#Comment:0"
        token.detach
        expected_tokens = ["#EOF", "#Comment", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 44
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:4>RampUp:0>#RampUpLine:0
    def match_token_at_45(self, token, context):
        ...
        state_comment = "State: 45 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:4>RampUp:0>#RampUpLine:0"
        token.detach
        expected_tokens = ["#EOF", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
        ...
        self.add_error(context, error)
        return 45
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:5>RampDown:0>#RampDownLine:0
    def match_token_at_46(self, token, context):
        ...
        state_comment = "State: 46 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:5>RampDown:0>#RampDownLine:0"
        token.detach
        expected_tokens = ["#EOF", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
        ...
        self.add_error(context, error)
        return 46
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:6>RandomWait:0>#RandomWaitLine:0
    def match_token_at_47(self, token, context):
        ...
        state_comment = "State: 47 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:1>SimulationPeriod:6>RandomWait:0>#RandomWaitLine:0"
        token.detach
        expected_tokens = ["#EOF", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
        ...
        self.add_error(context, error)
        return 47
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:0>#SimulationSampleLine:0
    def match_token_at_48(self, token, context):
        ...
        if self.match_Comment(context, token):
                self.build(context, token)
                return 50
        if self.match_PopulationLine(context, token):
                self.start_rule(context, 'Population')
                self.build(context, token)
                return 51
        if self.match_TimeLine(context, token):
                self.start_rule(context, 'Time')
        ...
        state_comment = "State: 48 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:0>#SimulationSampleLine:0"
        token.detach
        expected_tokens = ["#Empty", "#Comment", "#PopulationLine", "#TimeLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 48
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:1>Description_Helper:1>Description:0>#Other:0
    def match_token_at_49(self, token, context):
        ...
        state_comment = "State: 49 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:1>Description_Helper:1>Description:0>#Other:0"
        token.detach
        expected_tokens = ["#Comment", "#PopulationLine", "#TimeLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 49
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:1>Description_Helper:2>#Comment:0
    def match_token_at_50(self, token, context):
        ...
        state_comment = "State: 50 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:1>Description_Helper:2>#Comment:0"
        token.detach
        expected_tokens = ["#Comment", "#PopulationLine", "#TimeLine", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 50
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:0>#PopulationLine:0
    def match_token_at_51(self, token, context):
        if self.match_TableRow(context, token):
                self.start_rule(context, 'DataTable')
                self.build(context, token)
                return 52
        ...
        if self.match_PercentageLine(context, token):
                ...
                return 55
        if self.match_PopulationLine(context, token):
                self.end_rule(context, 'Population')
                self.start_rule(context, 'Population')
                self.build(context, token)
                return 51
        if self.match_TimeLine(context, token):
        ...
        state_comment = "State: 51 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:0>#PopulationLine:0"
        token.detach
        expected_tokens = ["#TableRow", "#DocStringSeparator", "#PercentageLine", "#PopulationLine", "#TimeLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 51
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:0>DataTable:0>#TableRow:0
    def match_token_at_52(self, token, context):
        ...
        state_comment = "State: 52 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:0>DataTable:0>#TableRow:0"
        token.detach
        expected_tokens = ["#TableRow", "#DocStringSeparator", "#PercentageLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 52
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:1>DocString:0>#DocStringSeparator:0
    def match_token_at_53(self, token, context):
        ...
        state_comment = "State: 53 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:1>DocString:0>#DocStringSeparator:0"
        token.detach
        expected_tokens = ["#DocStringSeparator", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
        ...
        self.add_error(context, error)
        return 53
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:1>DocString:2>#DocStringSeparator:0
    def match_token_at_54(self, token, context):
        if self.match_PercentageLine(context, token):
                self.end_rule(context, 'DocString')
                self.start_rule(context, 'Percentage')
                self.build(context, token)
        ...
        state_comment = "State: 54 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:1>DocString:2>#DocStringSeparator:0"
        token.detach
        expected_tokens = ["#PercentageLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 54
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:2>Percentage:0>#PercentageLine:0
    def match_token_at_55(self, token, context):
        if self.match_StartLine(context, token):
        ...
        state_comment = "State: 55 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:2>Percentage:0>#PercentageLine:0"
        token.detach
        expected_tokens = ["#StartLine", "#StopLine", "#SynchronizedLine", "#PopulationLine", "#TimeLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 55
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:3>Start:0>#StartLine:0
    def match_token_at_56(self, token, context):
        if self.match_StopLine(context, token):
                self.end_rule(context, 'Start')
                self.start_rule(context, 'Stop')
                self.build(context, token)
        ...
        state_comment = "State: 56 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:3>Start:0>#StartLine:0"
        token.detach
        expected_tokens = ["#StopLine", "#SynchronizedLine", "#PopulationLine", "#TimeLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 56
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:4>Stop:0>#StopLine:0
    def match_token_at_57(self, token, context):
        if self.match_SynchronizedLine(context, token):
                self.end_rule(context, 'Stop')
                self.start_rule(context, 'Synchronized')
                self.build(context, token)
        ...
        state_comment = "State: 57 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:4>Stop:0>#StopLine:0"
        token.detach
        expected_tokens = ["#SynchronizedLine", "#PopulationLine", "#TimeLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 57
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:5>Synchronized:0>#SynchronizedLine:0
    def match_token_at_58(self, token, context):
        if self.match_PopulationLine(context, token):
                self.end_rule(context, 'Synchronized')
                self.end_rule(context, 'Population')
                self.start_rule(context, 'Population')
        ...
        state_comment = "State: 58 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:2>Population:1>Population_Arg:5>Synchronized:0>#SynchronizedLine:0"
        token.detach
        expected_tokens = ["#PopulationLine", "#TimeLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 58
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:3>Time:0>#TimeLine:0
    def match_token_at_59(self, token, context):
        if self.match_Empty(context, token):
                self.build(context, token)
                return 59
        if self.match_Comment(context, token):
        ...
        state_comment = "State: 59 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:3>Time:0>#TimeLine:0"
        token.detach
        expected_tokens = ["#Empty", "#Comment", "#TotalRunnersLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 59
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:3>Time:1>Description_Helper:1>Description:0>#Other:0
    def match_token_at_60(self, token, context):
        if self.match_Comment(context, token):
                self.end_rule(context, 'Description')
                self.build(context, token)
                return 61
        ...
        state_comment = "State: 60 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:3>Time:1>Description_Helper:1>Description:0>#Other:0"
        token.detach
        expected_tokens = ["#Comment", "#TotalRunnersLine", "#Other"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 60
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:3>Time:1>Description_Helper:2>#Comment:0
    def match_token_at_61(self, token, context):
        if self.match_Comment(context, token):
                self.build(context, token)
                return 61
        if self.match_TotalRunnersLine(context, token):
        ...
        state_comment = "State: 61 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:3>Time:1>Description_Helper:2>#Comment:0"
        token.detach
        expected_tokens = ["#Comment", "#TotalRunnersLine", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 61
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:4>TotalRunners:0>#TotalRunnersLine:0
    def match_token_at_62(self, token, context):
        if self.match_EOF(context, token):
                self.end_rule(context, 'TotalRunners')
                self.end_rule(context, 'SimulationSample')
                self.end_rule(context, 'Simulation_Definition')
                self.end_rule(context, 'Plan')
                self.build(context, token)
                return 66
        ...
        state_comment = "State: 62 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:4>TotalRunners:0>#TotalRunnersLine:0"
        token.detach
        expected_tokens = ["#EOF", "#RampUpLine", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
            raise error
        self.add_error(context, error)
        return 62
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:5>RampUp:0>#RampUpLine:0
    def match_token_at_63(self, token, context):
        if self.match_EOF(context, token):
                self.end_rule(context, 'RampUp')
                self.end_rule(context, 'SimulationSample')
                self.end_rule(context, 'Simulation_Definition')
        ...
        state_comment = "State: 63 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:5>RampUp:0>#RampUpLine:0"
        token.detach
        expected_tokens = ["#EOF", "#RampDownLine", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
        ...
        self.add_error(context, error)
        return 63
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:6>RampDown:0>#RampDownLine:0
    def match_token_at_64(self, token, context):
        if self.match_EOF(context, token):
                self.end_rule(context, 'RampDown')
                self.end_rule(context, 'SimulationSample')
                self.end_rule(context, 'Simulation_Definition')
        ...
        state_comment = "State: 64 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:6>RampDown:0>#RampDownLine:0"
        token.detach
        expected_tokens = ["#EOF", "#RandomWaitLine", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):
        ...
        self.add_error(context, error)
        return 64
    # SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:7>RandomWait:0>#RandomWaitLine:0
    def match_token_at_65(self, token, context):
        if self.match_EOF(context, token):
                self.end_rule(context, 'RandomWait')
                self.end_rule(context, 'SimulationSample')
                self.end_rule(context, 'Simulation_Definition')
        ...
        state_comment = "State: 65 - SaladDocument:0>Plan:2>Simulation_Definition:1>__alt0:2>SimulationSample:7>RandomWait:0>#RandomWaitLine:0"
        token.detach
        expected_tokens = ["#EOF", "#TagLine", "#SimulationLine", "#SimulationPeriodLine", "#SimulationSampleLine", "#Comment", "#Empty"]
        error = UnexpectedEOFException(token, expected_tokens, state_comment) if token.eof() else UnexpectedTokenException(token, expected_tokens, state_comment)
        if (self.stop_at_first_error):

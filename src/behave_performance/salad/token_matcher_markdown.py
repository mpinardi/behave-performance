...
class SaladInMarkdownTokenMatcher(object):
    ...
    def reset(self):
        ...
        self.matched_Plan_line=False

    def match_PlanLine(self, token):

        if(self.matched_plan_line):
            self._set_token_matched(token,None)

        # We first try to match "# Plan: blah"
        result = self._match_title_line(KEYWORD_PREFIX_HEADER, self.dialect.plan_keywords, ':', token, 'PlanLine')
        # If we didn't match "# Plan: blah", we still match this line
        # as a PlanLine.
        # The reason for this is that users may not want to be constrained by having this as their fist line.

        if not result:
            self._set_token_matched(token,'PlanLine',token.line.get_line_text())
        self.matched_plan_line=result
        return result
        

    def match_SimulationLine(self, token):
        return self._match_title_line(KEYWORD_PREFIX_HEADER, self.dialect.simulation_keywords, ':', token, 'SimulationLine') or self._match_title_line(KEYWORD_PREFIX_HEADER, self.dialect.simulation_outline_keywords, ':', token, 'SimulationLine')
    ...
    def match_GroupLine(self, token):
        nonStarStepKeywords = (self.dialect.group_keywords)
        return self._match_title_line(KEYWORD_PREFIX_BULLET, nonStarStepKeywords, '', token, 'GroupLine')
    ...
    def match_Empty(self, token):
        ...
        if ( not self.match_TagLine(token) and 
             not self.match_PlanLine(token) and
             not self.match_SimulationLine(token) and
             ...
             not self.match_GroupLine(token)
           ):
            # neutered
            result = True

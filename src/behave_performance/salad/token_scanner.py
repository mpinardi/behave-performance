...
from .salad_line import SaladLine


class TokenScanner(object):
    """
    The scanner reads a Salad doc (typically read from a `.plan` file) and creates a token for
    each line.

    The tokens are passed to the parser, which outputs an AST (Abstract Syntax Tree).

    If the scanner sees a `#` language header, it will reconfigure itself dynamically to look for
    Salad keywords for the associated language. The keywords are defined in
    :file:`Salad-languages.json`.
    """
    ...
    def read(self):
        ...
        return Token((SaladLine(line, self.line_number) if line else line), location)
